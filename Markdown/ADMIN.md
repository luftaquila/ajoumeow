...
### 입력 유효성 검사
입력이 올바르지 않을 경우 데이터를 전송하지 않으며, 상단 상태 표시줄에 오류 정보를 표시합니다.
1. `ERR_WRONG_DATE`  
날짜를 선택하지 않았거나 날짜 패턴이 올바르지 않은 경우
1. `ERR_NO_PAYLOAD`  
...
> 확인서는 수원시 자원봉사센터에 제출합니다. 이메일 주소는 `sotong@suwonvol.com` 입니다.
* 스크립트 최초 실행 시 **확인되지 않은 앱** 알림이 표시될 수 있습니다.  
`고급` → `Receiver_미유미유(으)로 이동(안전하지 않음)`을 클릭해 실행합니다.
