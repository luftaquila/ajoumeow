...
* `기온`은 [OpenWeatherMap](https://openweathermap.org/city/1835553)의 수원시 기온을 섭씨 단위로 표시합니다. 매 10분마다 갱신됩니다.
...
* `날짜` 입력 필드를 클릭하면 대부분의 브라우저에서 달력이 표시됩니다. 원하는 날짜를 클릭하면 선택됩니다.
* `코스` 중 하나를 선택합니다.
* `신청하기` 버튼을 눌러 입력한 내용을 전송합니다.
...
* 페이지에서 [Rank]()를 누르면 `3일간 보지 않기` 옵션이 해제됩니다.
 ...
 * 학기 기간은 수동으로 세팅해야 합니다.
   ...
   + **M1** 셀에서 저번 학기 시작일을 선택합니다.
   + **M2** 셀에서 저번 학기 종료일을 선택합니다.
...
#### 1. 표준 입력
1. [Calc]()을 눌러 이동합니다.
* 데이터 로딩이 완료되면 관리자 비밀번호를 입력합니다.
* 날짜 선택 필드에서 날짜를 선택하면 해당일의 신청자가 모두 표시됩니다.
* **급식 인증에 이름이 있는 회원만** 체크박스가 선택된 상태로 둔 후, `Confirm` 버튼을 눌러 전송합니다.
...
* 데이터 로딩이 완료되면 관리자 비밀번호를 입력합니다.
* `수동 입력` 버튼을 클릭합니다.
* 이름과 지급할 점수를 입력하고 `Confirm` 버튼을 눌러 전송합니다.
