...
사이트의 사이드바(☰ 누르면 나오는 거) 하단에 [관리자 콘솔](#) 링크를 눌러 이동합니다.
...
* 급식표에서 급식 신청 삭제는 오늘로부터 2주 안에 내가 신청한 것만 삭제할 수 있지만, 관리자 권한은 이를 무시하고 지난 날짜 또는 내가 신청하지 않은 급식 신청을 모두 삭제할 수 있습니다.
* 관리자 콘솔에 접속할 수 있습니다.

??? : 어마어마한 특권이네요! 그럼 관리자 권한은 어떻게 받죠?
* 새 관리자 등록은 기존 임원진만 할 수 있습니다. 임원진이 모두 바뀌게 된다면 🔥반드시🔥 신규 임원진에게 관리자 권한을 부여하고 가세요.
* 관리자 권한은 콘솔의 `회원 관리` 탭에서 `직책`이 `회원`이 아닌 모든 사람에게 부여됩니다.
* 새 임원진이 먼저 일반 회원으로 등록한 상태일 때, 기존 임원진이 `회원 관리` 탭에서 직책을 변경하세요.
...
급식 인증은 시스템에 활동했다는 사실을 기록하는 것입니다.
...
### 급식 인증
  ...
  1. 인증하고자 하는 회원(**카톡 급식인증방에 인증한 회원**)만 좌측의 체크박스에 체크합니다.
      * 기본값은 다 체크된 상태입니다. 실제로 인증하지 않은 회원은 체크를 해제하세요.
      * 실제 급식을 했지만 신청하지 않아 목록에 표시되지 않을 경우 `+ 추가` 를 눌러 수동으로 인증 기록을 추가합니다.
...
* `+ 추가`를 눌러 입력 필드를 추가합니다.
* 인원이 둘 이상일 때 `>지급 사유 통일하기`를 누르면 모든 지급 사유가 첫 번째 항목의 사유로 자동 입력됩니다.

### 인증 삭제
이미 인증한 기록을 삭제합니다.
* 삭제할 기록이 있는 날짜를 선택하면 해당일에 인증된 모든 기록이 표시됩니다.
* 삭제할 기록만 체크한 후, `삭제` 버튼을 클릭합니다.

### 입력 유효성 검사
입력이 올바르지 않을 경우 데이터를 전송하지 않으며, 오류 정보가 표시됩니다.
1. `ERR_NO_PAYLOAD`  
체크된 항목이 단 한 개도 존재하지 않을 경우
1. `ERR_INVALID_PAYLOAD`  
체크된 항목 중 비어있는 입력 필드가 한 개 이상 존재할 경우
...
* `회원 등록` 및 `신입 모집`은 설정된 기간을 만족하거나 추가 허용 여부가 TRUE일 때, 둘 중 하나만 만족해도 활성화됩니다.
...
⚠️ 모집을 시작하기 전에 반드시 `설정` 탭에서 `현재 학기` 및 `신입 모집` 관련 설정을 업데이트하세요.

모집 전에 `현재 학기`를 최신화해야 신청서가 학기별로 제대로 분류됩니다.
...
* Google 연락처 파일을 사용하는 경우
  ...
  1. 좌측 상단 메뉴에서 가져오기 클릭
  1. 다운로드한 연락처 파일 선택 후 가져오기 클릭
  1. 핸드폰에서 연락처 동기화하면 [년도]-[학기] 미유미유 라벨에 [학번][이름] 형식으로 가입신청자 연락처가 저장됩니다.
  1. 카톡방 초대가 끝나면 라벨을 삭제해서 간단하게 제거할 수 있습니다.
...
서버 로그를 열람합니다.
* 체크된 범주의 로그만 표시합니다.
로그의 대부분이 페이지를 로딩할 때 발생하기 때문에, 페이지 로드 체크박스를 해제하면 보다 중요한 로그만 볼 수 있습니다.
* 오류 체크박스에 체크하면 오류 기록만 표시됩니다.
체크하지 않으면 일반 로그와 오류 로그 모두 표시합니다.
