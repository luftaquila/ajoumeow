$(function() {
  $.ajax({
    ...
    success: function(res) {
      if(!res.result) window.location.href = 'https://luftaquila.io/403.html';
      $.ajax({
        url:"https://luftaquila.io/ajoumeow/api/loginCheck",
        type: "POST",
        dataType: 'json',
        success: function(response) {
          if(response.name) $('#welcomeMSG').text(response.name + '(관리자)님 안녕하세요.');
        }
      });
    },
    error: function(res) {
...
function onLoad() {
  ...
  $.when(requestVerifyData(), requestLatestVerify()).done(function(res1, res2) {
    $('#DATA input, #DATA select, #DATA a').attr('disabled', false);
  ...
  $.ajax({ // Request Sheet Lists
    ...
    success: function(response) {
      ...
      $('#recruitContent input, #recruitContent select').attr('disabled', false);
    }
  });
  ...
  $('#dataTable th').css('width', '100px!important');
}
...
function clickEventListener() {
  ...
  $('#add').click(function() {
    ...
    div.append($('<input id="addInput_' + addCount + '" style="width: 50!important" />')).append(' / ');
  ...
  $('#mnadd').click(function() {
    ...
    div.append('이름 : ').append($('<input id="mnAddName_' + mnAddCount + '" style="width: 50!important" />')).append('&nbsp;&nbsp;');
    div.append('점수 : ').append($('<input type="number" id="mnAddScore_' + mnAddCount + '" style="width: 30!important" />')).append('&nbsp;&nbsp;');
    div.append('지급 사유 : ').append($('<input id="mnAddReason_' + mnAddCount + '" style="width: 80!important" />')).append('&nbsp;&nbsp;');
  });
