...
async function init() {
  ...
  await $.ajax({
    ...
    success: res => { record = res.data.records; verify = res.data.verifications },
    error: err => alertify.error(`${err.responseJSON.error.message}`)
  });
  ...
  for(let obj of record) $('#autolist').append("<div><label><input type='checkbox' name='recordList' checked='checked' value='" + obj.studentId + '/' + obj.name + '/' + obj.course + "'>&nbsp;" + obj.studentId + ' ' + obj.name + ' / ' + obj.course + "</input></label></div>");
  $('#deletelist').html('<br>');
  for(let obj of verify) $('#deletelist').append("<div><label><input type='checkbox' name='deleteList' value='" + obj.studentId + '/' + obj.name + '/' + obj.course + "'>&nbsp;" + obj.studentId + ' ' + obj.name + ' / ' + obj.course + ' ' + obj.score + "점</input></label></div>");
}
...
function autoadd_typeahead() {
  $('input.autoCompleteName').typeahead({
    ...
    templates: {
      suggestion: data => {
        return `<p>${data.studentId} <strong>${data._query}</strong>${data.name.replace(data._query, '')}</p>`;
      }
    },
  ...
  .bind('typeahead:select', (e, suggestion) => {
    $(e.target).parent().siblings('span').text(`${suggestion.studentId} ${suggestion.name}`)
    $(e.target).parent().siblings('input[name=recordList]').val(`${suggestion.studentId}/${suggestion.name}/${$(e.target).parent().siblings('select').val()}`)
  });
...
function validator(payload) {
  ...
  for(let obj of payload) {
    if(!obj.studentId || !obj.date || !obj.name || !obj.course) {
