    ...
    $.ajax({ // Request settings
        ...
        success: function(response) {
            ...
            if( (new Date() > new Date(assignDate[0]) && new Date() < new Date(new Date(assignDate[1]).getTime() + 60 * 60 * 24 * 1000)) || (settingList[4][1] == 'true')) {
                ...
                $('.loading').html('<br><br><br><h4>모집 기간이 아닙니다!<br>다음 모집 때 지원해 주세요 ㅠㅠ</h4><br><br><img src="images/failed.jpg" style="width:90%">');
            }
                
        }
    });

    var collegeList = '', departmentList = '';
    ...
    $('.validate-form').on('submit', function(event) {
        ...
        if(check) {
            ...
            $.ajax({
                ...
                error: function(response) { alertify.error('ERR::REGISTER_FAILURE<br>관리자에게 문의하세요.'); }
            });
